...
this statement is used to call one constructor from another constructor which are present in the same class.

                        Constructor chaining in same class
It is a process of one constructor calling another constructor within the same class  using this() is called as constructor chaining. 

note:
-> if a constructor chaining leads to a loop then compiler throws an recursive constructor invocation error at compile time. 
-> this() should always be written in the first line of the constructor body.
-> Multiple constructors cannot be called from a single constructor body.
-> this() cannot be used to a constructor from itself,v=beacuse it leads to ecursive constructor invocation error at compile time. 

Application
-> It reduces multiple objects creation to invoke multiple overloaded constructors at the same time.
-> It reduces the burden on a constructor when it has lot of process to undergo by spliting the tasks to multiple constructors.
