class Alibaba{
	...
	*/
	...
	public void addToCart(int itemId){
       ...
       {
                itemIds[i] = itemId;
                System.out.println("counter entry"+i); 
                counter+=1; 
        }
	...
	public void makePayment(DebitCard dcd){
        if (dcd.cvv != 0) {
        	if(paymentFlag == false){ 
        	      System.out.println("Total amount to be paid using DebitCard  = "+(counter*1000));
                  paymentFlag = true;
              }else{
	...
	public void makePayment(CreditCard ccd){
		if(ccd.cvv != 0){ 
               if(paymentFlag == false){ 
        	      System.out.println("Total amount to be paid using CreditCard  = "+(counter*1000));
                  paymentFlag = true;
              }else{
              	System.out.println("Payment is already done");
              }
